---
description: Derive a compact handoff for a portable workflow without changing it.
...
Treat `$ARGUMENTS` as an optional explicit workflow id or package path. Resolve the sole nonterminal workflow only when the target is omitted. Derive the handoff from canonical files and do not save it as new state.
