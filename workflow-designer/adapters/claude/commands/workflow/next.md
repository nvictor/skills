---
description: Report the next valid action for a portable workflow without performing it.
...
Treat `$ARGUMENTS` as an optional explicit workflow id or package path. Resolve the sole nonterminal workflow only when the target is omitted. Do not perform domain work.
