...
from validate_workflow_package import validate_manifest
...
class PackageNameTests(unittest.TestCase):
    def errors_for(self, name, missing=False, extra=None):
        with tempfile.TemporaryDirectory() as directory:
            ...
            template = Path(__file__).resolve().parents[1] / "assets/workflow-package"
    ...
    def test_accepts_canonical_names(self):
        for name in ("Workflow: Example", "Workflow: SRE", "Workflow: X"):
            with self.subTest(name=name):
                self.assertEqual([], self.errors_for(name))

    def test_rejects_invalid_names(self):
        for name in ("Example", "Other: Example", "workflow: Example", "Workflow:",
                     "Workflow: ", "Workflow:   ", "Workflow:  Example", "Workflow: Example ",
    ...
    def test_rejects_conversation_discriminator(self):
        errors = self.errors_for("Workflow: Example", extra={"conversation_file": "conversation.md"})
