...
try {
  ...
  if (fs.existsSync(tmpFile)) {
    startTime = parseInt(fs.readFileSync(tmpFile, 'utf-8'), 10);
  }

  const durationSec = startTime > 0 ? Math.round((Date.now() - startTime) / 1000) : 0;
  const prompt = input.prompt || 'Unknown task';
  ...
  const logEntry = `- **[${inferCategory(prompt)}]** (${durationSec}s) - ${timestampStr}: ${truncateDescription(prompt)}\n`;

  if (!fs.existsSync(logFile)) {
    fs.writeFileSync(logFile, '# Task Log\n\n');
  }
  ...
  console.error('Error in after-agent hook:', e.message);
}
