...
const STATE_DIR = path.join(os.tmpdir(), 'codex-task-logger');
...
function statePath(sessionId, turnId) {
  const safeSession = (sessionId || 'unknown-session').replace(/[^a-zA-Z0-9_-]/g, '_');
  const safeTurn = (turnId || 'unknown-turn').replace(/[^a-zA-Z0-9_-]/g, '_');
  return path.join(STATE_DIR, `${safeSession}_${safeTurn}.json`);
}
